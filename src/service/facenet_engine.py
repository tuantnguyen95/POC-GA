...
import traceback
import http.client
from service.element_finder import ElementFinderByXML, ElementFinder
from service.logger import LoggingWrapper
import os

facenet_engine = Blueprint('facenet_engine', __name__)
logger = LoggingWrapper()


@facenet_engine.route('/')
def index():
    return 'Serving for Facenet engine'
...
@facenet_engine.route(constants.ELEMENT_FINDING_URL, methods=['POST'])
def find_element():
    ...
    serving_host = os.getenv('KOBITON_AI_TF_SERVING_HOST')
    serving_port = os.getenv('KOBITON_AI_TF_SERVING_PORT')
    services = {constants.TENSORFLOW_SERVING_SERVICE:'%s:%s' % (serving_host, serving_port)}
    ...
    try:
        ...
        solver = ElementFinderByXML(prime_sess, revisit_sess, finder.logger, finder._try_get_visual_embeddings,
                                     session_id=session_id,
                                     action_id=action_id, request_id=request_id,
                                     classifier_model=finder.sk_cls)
        cand_xpaths, cand_sims = solver.find_element_by_facenet()
        response = {
            'elements': [{"xpath": cand[0], "confidence": cand[1]} for cand in zip(cand_xpaths[-5:], cand_sims[-5:])]}
    except Exception:
